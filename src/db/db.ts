...
export class KwentaDB extends Dexie {
  ...
  constructor() {
    ...
    // Version 14: add optional `method` (how a payment moved: cash / transfer / …) to
    // settlements. Non-indexed, so the stores schema is unchanged; backfill existing rows
    // to null so the field is present everywhere.
    this.version(14).stores({
      ...
      await tx.table('settlements').toCollection().modify((s: Record<string, unknown>) => {
        if (s.method === undefined) s.method = null
      })
    })

    // Version 15: `pending_mutations` becomes an ordered write queue. Each entry carries the exact
    // rows it submits (`push`), the rows it owns (`row_keys`, multi-entry), its replay order
    // (`seq`) and the submission id every attempt reuses. Legacy entries keep `push = null` and
    // stay on the row-scan safety net; finished legacy history (applied/dismissed) is dropped.
    this.version(15).stores({
      ...
      profile_peer_links:
        'id, owner_user_id, anchor_profile_id, peer_profile_id, synced_at, is_deleted, [owner_user_id+anchor_profile_id], [owner_user_id+is_deleted]',
      pending_mutations:
        'id, actor_user_id, status, entity_type, entity_id, created_at, updated_at, seq, submission_id, *row_keys, [actor_user_id+status+seq]',
      not_applied_changes:
        'id, actor_user_id, resolution, entity_type, entity_id, [entity_type+entity_id], created_at, resolved_at, pending_mutation_id',
    }).upgrade(async (tx) => {
      const table = tx.table('pending_mutations')
      const rows = (await table.toArray()) as Record<string, unknown>[]
      const finished = rows.filter((r) => r.status === 'applied' || r.status === 'dismissed')
      if (finished.length > 0) await table.bulkDelete(finished.map((r) => r.id as string))
      const live = rows
        .filter((r) => r.status !== 'applied' && r.status !== 'dismissed')
        .sort((a, b) => String(a.created_at ?? '').localeCompare(String(b.created_at ?? '')))
      const upgraded = live.map((r, i) => {
        const { idempotency_key: idempotencyKey, ...rest } = r
        return {
          ...rest,
          seq: i + 1,
          submission_id: typeof idempotencyKey === 'string' && idempotencyKey ? idempotencyKey : crypto.randomUUID(),
          push: null,
          row_keys: [],
