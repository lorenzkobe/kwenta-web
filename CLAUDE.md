...
3. **Migrations are append-only.** Next number: **`073`**. Never edit a past migration.
...
5. **A function that names its caller must not be callable by the caller.** Client-facing
   ...
   with `SELECT public.kwenta_revoke_acting_user_helpers();`. The money helpers in 052–054/057/063 read base
