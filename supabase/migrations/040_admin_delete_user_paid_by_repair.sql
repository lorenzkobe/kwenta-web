-- Fix a foreign-key violation in admin_delete_user.
--
-- bills.paid_by (migration 032) is NOT NULL with the default FK action (NO ACTION).
-- admin_delete_user collects v_bill_ids only for bills the target created or that
-- belong to a group the target created. A GROUP bill created by another user, in a
-- group the target did not create, but PAID BY the target, is never collected — so the
-- final `DELETE FROM public.profiles` fails with a foreign-key violation (the paid_by
-- reference still points at the row being deleted) and the whole admin delete aborts.
--
-- Fix: before deleting profiles, reassign paid_by to that bill's creator for any bill
-- NOT being deleted that still references a to-be-deleted profile. (item_splits.user_id
-- and settlements.from/to_user_id are already collected for deletion, so only paid_by
-- can dangle.)
...
BEGIN
  ...
  -- Repair paid_by on bills that survive the delete but reference a deleted profile,
  -- so the NOT NULL / NO ACTION FK on bills.paid_by doesn't block the profile delete.
  UPDATE public.bills
  SET
    paid_by = created_by,
    updated_at = now()
  WHERE paid_by = ANY(v_profile_ids)
    AND NOT (id = ANY(v_bill_ids));
