-- Fix operator-precedence bug in kwenta_push_settlements.
--
-- The previous WHERE clause was structured as:
--     ( group_settlement_allowed )            -- A
--   OR ( personal_settlement_allowed )         -- B
--   AND ( bill_id IS NULL OR bill visible )    -- C
--
-- Because AND binds tighter than OR, Postgres parsed this as `A OR (B AND C)`,
-- so the bill-visibility guard (C) applied ONLY to personal settlements. A group
-- member could push a settlement with bill_id pointing at ANY bill — including one
-- in another group or a bill they cannot see — polluting that bill's settled math.
--
-- Fix: parenthesize the OR group so C applies to both branches: `(A OR B) AND C`.
