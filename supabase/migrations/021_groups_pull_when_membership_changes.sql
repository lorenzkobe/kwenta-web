-- Incremental sync previously only pulled `groups` rows when `groups.updated_at > p_since`.
-- Adding a member updates `group_members` but often leaves `groups.updated_at` unchanged,
-- so new members received `group_members` locally without the parent `groups` row
-- ("Group not found", empty Groups list). Include groups when the *user's membership*
-- row changed since the last pull.
